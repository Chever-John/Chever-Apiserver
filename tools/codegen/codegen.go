...
！！CAS 系统错误码列表，由 {{.}}codegen -type=int -doc{{.}} 命令生成，不要对此文件做任何更改。
...
// handleError handle error and output those error to stderr, then exit with code 2.
...
func Usage() {
	if _, err := fmt.Fprintf(os.Stderr, "Usage of codegen:\n"); err != nil {
		handleError(err)
	}

	if _, err := fmt.Fprintf(os.Stderr, "\tcodegen [flags] -type T [directory]\n"); err != nil {
		handleError(err)
	}

	if _, err := fmt.Fprintf(os.Stderr, "\tcodegen [flags] -type T files... # Must be a single package\n"); err != nil {
		handleError(err)
	}
...
func main() {
	...
	exampleTypes := strings.Split(*typeNames, ",")
	var tags []string
	if len(*buildTags) > 0 {
	...
	var src []byte
	for _, typeName := range exampleTypes {
		if *doc {
...
type Value struct {
	...
	// whether to interpret it as an int64 or uint64; the only place
